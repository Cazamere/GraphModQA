...
def few(final_graph, final_graph_str, augment_tasks, init_prompt, end_matrix_prompt, i, prompt_type, examples, examples_strs, few=True):
    ...
    if prompt_type == "add_edge":
        ...
        for n in range(num_examples):
            prompt_n = add_edge(examples[n], examples_strs[n], init_prompt, end_matrix_prompt, i, False, "", cot = True, few=True)
        ...
        solution_filename = f"data/ablation_few/solutions/add_edge/{num_examples}/solution_{i}.txt"
        ...
        for n in range(num_examples):
            prompt_n = remove_edge(examples[n], examples_strs[n], init_prompt, end_matrix_prompt, i, False, "", cot = True, few=True)
            full_example_prompt += prompt_n + '\n'
        ...
        # Write new graph to file
        solution_filename = f"data/ablation_few/solutions/remove_edge/{num_examples}/solution_{i}.txt"
        with open(solution_filename, "w") as solution_file:
            solution_file.write(new_graph_str)
        ...
        for n in range(num_examples):
            prompt_n = add_node(examples[n], examples_strs[n], init_prompt, end_matrix_prompt, i, False, "", cot = True, few=True)
            full_example_prompt += prompt_n + '\n'
        ...
        # Write new graph to file
        solution_filename = f"data/ablation_few/solutions/add_node/{num_examples}/solution_{i}.txt"
        with open(solution_filename, "w") as solution_file:
            solution_file.write(new_graph_str)
        ...
        for n in range(num_examples):
            prompt_n = remove_node(examples[n], examples_strs[n], init_prompt, end_matrix_prompt, i, False, "", cot = True, few=True)
            full_example_prompt += prompt_n + '\n'
        ...
        # Write new graph to file
        solution_filename = f"data/ablation_few/solutions/remove_node/{num_examples}/solution_{i}.txt"
        with open(solution_filename, "w") as solution_file:
            solution_file.write(new_graph_str)
        ...
        # Create prompt string
        cycle_prompt = f"Q: Does the resulting graph contain a cycle? Only write 'Yes' or 'No'.\n"
...
def cot(final_graph, final_graph_str, augment_tasks, init_prompt, end_matrix_prompt, i, prompt_type, examples, examples_strs):
    num_examples = len(examples)
    full_example_prompt = ''
    ...
    if prompt_type == "add_edge":
        ...
        # Write new graph to file
        solution_filename = f"data/ablation_cot/solutions/add_edge/{num_examples}/solution_{i}.txt"
        with open(solution_filename, "w") as solution_file:
            solution_file.write(new_graph_str)
        ...
        # Write new graph to file
        solution_filename = f"data/ablation_cot/solutions/remove_edge/{num_examples}/solution_{i}.txt"
        with open(solution_filename, "w") as solution_file:
            solution_file.write(new_graph_str)
        ...
        # Write new graph to file
        solution_filename = f"data/ablation_cot/solutions/add_node/{num_examples}/solution_{i}.txt"
        with open(solution_filename, "w") as solution_file:
            solution_file.write(new_graph_str)
        ...
        # Write new graph to file
        solution_filename = f"data/ablation_cot/solutions/remove_node/{num_examples}/solution_{i}.txt"
        with open(solution_filename, "w") as solution_file:
            solution_file.write(new_graph_str)
    elif prompt_type == "node_count":
        for n in range(num_examples):
...
def add_edge(graph, graph_str, init_prompt, end_prompt, i, part_of_chain, ablation_dir = "", ablationType = None, cot = False, graph_type = 'adjacency', encoding_dict = None, few = False):
    ...
    if part_of_chain:
        ...
        return full_add_edge_prompt
    else:
        #print(f'ablType: {ablationType}')
        #sys.exit(1)
        # Create prompt string
        if ablationType == "d": # directed
            add_edge_prompt = f"Q: Add an edge from node {node_a} to node {node_b}. Only write the resulting adjacency matrix.\n"
        elif ablationType == "few": # few-shot
            example_prompt = f""
        elif ablationType == "no_force":
            add_edge_prompt = f"Q: Add an edge between nodes {node_a} and {node_b}, and write the resulting adjacency matrix.\n"
        else:
            if graph_type == 'incidence':
                add_edge_prompt = f"Q: Add an edge between nodes {node_a} and {node_b}. Only write the resulting incidence graph.\n"
                ...
                add_edge_prompt = f"Q: Add an edge between authors {encoding_dict[node_a]} and {encoding_dict[node_b]}. Only write the resulting coauthorship graph.\n"
            elif graph_type == 'friendship':
                add_edge_prompt = f"Q: Add an edge between {encoding_dict[node_a]} and {encoding_dict[node_b]}. Only write the resulting friendship graph.\n"
            elif graph_type == 'social_network':
                add_edge_prompt = f"Q: Add an edge between {encoding_dict[node_a]} and {encoding_dict[node_b]}. Only write the resulting social network graph.\n"
        full_add_edge_prompt = init_prompt + graph_str + "\n" + add_edge_prompt + end_prompt
        ...
        # Write new graph to file
        solution_filename = f"data/{ablation_dir}solutions/{graph_type}/add_edge/solution_{i}.txt" if graph_type != 'adjacency' else f"data/{ablation_dir}solutions/add_edge/solution_{i}.txt"
        with open(solution_filename, "w") as solution_file:
            solution_file.write(new_graph_str)

        return add_edge_graph, add_edge_prompt

def graph_to_string_encoder(graph, graph_type, encoding_dict):
    ...
    for node in graph.nodes:
        ...
        if graph_type == "incidence":
            ...
            if len(neighbors) == 0:
                encoding_graph_str += f"{encoding_dict[node]} has not co-authored any papers.\n"
            else:
                for neighbor in neighbors:
                    encoding_graph_str += f"{encoding_dict[node]} and {encoding_dict[neighbor]} wrote a paper together.\n"

        elif graph_type == "friendship":
            if len(neighbors) == 0:
                encoding_graph_str += f"{encoding_dict[node]} has no friends.\n"
            else:
                for neighbor in neighbors:
                    encoding_graph_str += f"{encoding_dict[node]} and {encoding_dict[neighbor]} are friends.\n"

        elif graph_type == "social_network":
            if len(neighbors) == 0:
                encoding_graph_str += f"{encoding_dict[node]} has no connections.\n"
            else:
                for neighbor in neighbors:
                    encoding_graph_str += f"{encoding_dict[node]} and {encoding_dict[neighbor]} are connected.\n"
...
def remove_edge(graph, graph_str, init_prompt, end_prompt, i, part_of_chain, ablation_dir = "", ablationType = None, cot = False, graph_type = 'adjacency', encoding_dict = None, few = False):
    ...
    if part_of_chain:
        ...
        node_a, node_b = edge
        if ablationType == "d": # directed
            remove_edge_prompt = f"Q: Remove the edge from node {node_a} to node {node_b}. Only write the resulting adjacency matrix.\n"
        elif ablationType == "no_force":
            remove_edge_prompt = f"Q: Remove the edge between noden {node_a} and node {node_b}, and write the resulting adjacency matrix.\n"
        else:
            if graph_type == 'incidence':
                remove_edge_prompt = f"Q: Remove the edge between nodes {node_a} and {node_b}. Only write the resulting incidence graph.\n"
            elif graph_type == 'adjacency':
                remove_edge_prompt = f"Q: Remove the edge between node {node_a} and node {node_b}. Only write the resulting adjacency matrix.\n"
            elif graph_type == 'coauthorship':
                remove_edge_prompt = f"Q: Remove the edge between authors {encoding_dict[node_a]} and {encoding_dict[node_b]}. Only write the resulting coauthorship graph.\n"
            elif graph_type == 'friendship':
                remove_edge_prompt = f"Q: Remove the edge between friends {encoding_dict[node_a]} and {encoding_dict[node_b]}. Only write the resulting friendship graph.\n"
            elif graph_type == 'social_network':
                remove_edge_prompt = f"Q: Remove the edge between {encoding_dict[node_a]} and {encoding_dict[node_b]}. Only write the resulting social network graph.\n"
        ...
        # Write new graph to file
        solution_filename = f"data/{ablation_dir}solutions/{graph_type}/remove_edge/solution_{i}.txt" if graph_type != 'adjacency' else f"data/{ablation_dir}solutions/remove_edge/solution_{i}.txt"
        with open(solution_filename, "w") as solution_file:
            solution_file.write(new_graph_str)
...
def add_node(graph, graph_str, init_prompt, end_prompt, i, part_of_chain, ablation_dir = "", ablationType = None, cot = False, graph_type = 'adjacency', encoding_dict = None, few = False):
    ...
    if part_of_chain:
        ...
        if ablationType == "no_force":
            ...
            add_node_prompt = f"Q: Add a node to the graph. Only write the resulting incidence graph.\n"
        elif graph_type == 'adjacency':
            add_node_prompt = f"Q: Add a node to the graph. Only write the resulting adjacency matrix.\n"
        elif graph_type == 'coauthorship':
            add_node_prompt = f"Q: Add a node called {encoding_dict[number_of_nodes]} to the graph. Only write the resulting coauthorship graph.\n"
        elif graph_type == 'friendship':
            add_node_prompt = f"Q: Add a node called {encoding_dict[number_of_nodes]} to the graph. Only write the resulting friendship graph.\n"
        elif graph_type == 'social_network':
            add_node_prompt = f"Q: Add a node called {encoding_dict[number_of_nodes]} to the graph. Only write the resulting social network graph.\n"
        # Create prompt string
       # add_node_prompt = f"Q: Add a node to the graph. Only write the resulting adjacency matrix.\n"
        full_add_node_prompt = init_prompt + graph_str + "\n" + add_node_prompt + end_prompt
        ...
        # Write new graph to file
        solution_filename = f"data/{ablation_dir}solutions/{graph_type}/add_node/solution_{i}.txt" if graph_type != 'adjacency' else f"data/{ablation_dir}solutions/add_node/solution_{i}.txt"
        with open(solution_filename, "w") as solution_file:
            solution_file.write(new_graph_str)
...
def remove_node(graph, graph_str, init_prompt, end_prompt, i, part_of_chain, ablation_dir = "", ablationType = None, cot = False, graph_type = 'adjacency', encoding_dict = None, few = False):
    ...
    if part_of_chain:
        ...
        if ablationType == "no_force":
            ...
            remove_node_prompt = f"Q: Remove node {node} from the graph. Only write the resulting incidence graph.\n"
        elif graph_type == 'adjacency':
            remove_node_prompt = f"Q: Remove node {node} from the graph. Only write the resulting adjacency matrix.\n"
        elif graph_type == 'coauthorship':
            remove_node_prompt = f"Q: Remove node {encoding_dict[node]} from the graph. Only write the resulting coauthorship graph.\n"
        elif graph_type == 'friendship':
            remove_node_prompt = f"Q: Remove node {encoding_dict[node]} from the graph. Only write the resulting friendship graph.\n"
        elif graph_type == 'social_network':
            remove_node_prompt = f"Q: Remove node {encoding_dict[node]} from the graph. Only write the resulting social network graph.\n"
        ...
        # Write new graph to file
        solution_filename = f"data/{ablation_dir}solutions/{graph_type}/remove_node/solution_{i}.txt" if graph_type != 'adjacency' else f"data/{ablation_dir}solutions/remove_node/solution_{i}.txt"
        with open(solution_filename, "w") as solution_file:
            solution_file.write(new_graph_str)
...
def chain_same(graph, graph_str, task, static_tasks, init_prompt, end_prompt, i, final_task, chain_length):
    # ----------------------------
    # --- Chain  ---
    # ----------------------------
    ...
    if final_task == "node_count":
        ...
        # Save prompt to file
        prompt_filename = f"data/chains_same/{final_task}/{task}/{chain_length}/prompts/prompt_{i}.txt"
        with open(prompt_filename, "w") as prompt_file:
            prompt_file.write(full_chain_prompt)

        # Save solution to file
        solution_filename = f"data/chains_same/{final_task}/{task}/{chain_length}/solutions/solution_{i}.txt"
        with open(solution_filename, "w") as solution_file:
            solution_file.write(str(node_count))
...
def chain_same_example(graph, graph_str, task, static_tasks, init_prompt, end_prompt, i, final_task, chain_length):
    # ----------------------------
    # --- Chain  ---
    # ----------------------------
...
def chain_same_cot(graph, graph_str, task, static_tasks, init_prompt, end_prompt, i, final_task, chain_length, examples, examples_strs):
    ...
    example_chain_prompt = chain_same_example(examples[0], examples_strs[0], task, static_tasks, init_prompt, end_prompt, i, final_task, chain_length)
    example_chain_prompt += chain_same_example(examples[1], examples_strs[1], task, static_tasks, init_prompt, end_prompt, i, final_task, chain_length)
    ...
    if final_task == "node_count":
        ...
        # Save prompt to file
        prompt_filename = f"data/chains_same_cot/{final_task}/{task}/{chain_length}/prompts/prompt_{i}.txt"
        with open(prompt_filename, "w") as prompt_file:
            prompt_file.write(full_chain_prompt)

        # Save solution to file
        solution_filename = f"data/chains_same_cot/{final_task}/{task}/{chain_length}/solutions/solution_{i}.txt"
        with open(solution_filename, "w") as solution_file:
            solution_file.write(str(node_count))
...
def node_degree(graph, graph_str, init_prompt, end_prompt, i, ablation_dir = "", ablationType = None, cot = False, graph_type = 'adjacency', encoding_dict = None, few = False):
    ...
    if cot:
        ...
        if graph_type == 'incidence':
            ...
            node_degree_prompt = f"Q: How many papers does {encoding_dict[node]} have?\n"
        elif graph_type == 'friendship':
            node_degree_prompt = f"Q: How many friends does {encoding_dict[node]} have?\n"
        elif graph_type == 'social_network':
            node_degree_prompt = f"Q: How many connections does {encoding_dict[node]} have?\n"
...
def edge_exists(graph, graph_str, init_prompt, end_prompt, i, ablation_dir = "", ablationType = None, cot = False, graph_type = 'adjacency', encoding_dict = None, few = False):
    ...
    if cot:
        ...
        return full_edge_exists_prompt
    else:
        if ablationType == "no_force":
            ...
            edge_exists_prompt = f"Q: Have {encoding_dict[node_a]} and {encoding_dict[node_b]} written a paper together? Only write 'Yes' or 'No'.\n"
        elif graph_type == 'friendship':
            edge_exists_prompt = f"Q: Are {encoding_dict[node_a]} and {encoding_dict[node_b]} friends? Only write 'Yes' or 'No'.\n"
        elif graph_type == 'social_network':
            edge_exists_prompt = f"Q: Are {encoding_dict[node_a]} and {encoding_dict[node_b]} connected? Only write 'Yes' or 'No'.\n"
...
def connected_nodes(graph, graph_str, init_prompt, end_prompt, i, ablation_dir = "", ablationType = None, cot = False, graph_type = 'adjacency', encoding_dict = None, few = False):
    ...
    if cot:
        ...
        return full_connected_nodes_prompt
    else:
        if graph_type == 'incidence':
            ...
            connected_nodes_prompt = f"Q: List all coauthors of {encoding_dict[node]}.\n"
        elif graph_type == 'friendship':
            connected_nodes_prompt = f"Q: List all friends of {encoding_dict[node]}.\n"
        elif graph_type == 'social_network':
            connected_nodes_prompt = f"Q: List all connections of {encoding_dict[node]}.\n"
        # Create prompt string
        full_connected_nodes_prompt = init_prompt + graph_str + "\n" + connected_nodes_prompt + end_prompt
        ...
        with open(solution_filename, "w") as solution_file:
            if graph_type == 'adjacency':
                ...
                connected_nodes = sorted([encoding_dict[node_b] for node_b in graph.neighbors(node)])
            solution_file.write(str(connected_nodes))
